...
# EH - change the branch name
            self.jetBranchName = "selectedPatJetsAK8PFPuppi"
            self.subJetBranchName = "selectedPatJetsAK8PFPuppiSoftDrop_Subjets"
...
# EH - added jet-subjet matching           
            SubJetMatcher = matchObjectCollectionMultiple(jets,
                                                          subJets,
                                                          dRmax=0.8)
            if not self.isData:
        ...
        for jet in jets:
            ...
            if self.doGroomed:
                if not self.isData:
                    ...
                    genGroomedJet = None
# EH - modified the groomed subjets to use the matched subjets
                GroomedSubJets = SubJetMatcher[
                    jet] if jet is not None else None
                # check subjet jecs
                groomedP4 = GroomedSubJets[0].p4(
                ) + GroomedSubJets[1].p4(
                )  if GroomedSubJets is not None and len(
                    GroomedSubJets) >= 2 else None
...
#                    groomedP4 = None
                ...
                if not self.isData:
                    ...
                    elif self.era in ["UL2018"]:	#copying 2018 for now
                        jmstau21DDTNomVal = 1.000  # tau21DDT < 0.43 WP
                        jmstau21DDTDownVal = 0.990
                        jmstau21DDTUpVal = 1.010
                        self.jetSmearer.jmr_vals = [1.124, 1.208, 1.040]
